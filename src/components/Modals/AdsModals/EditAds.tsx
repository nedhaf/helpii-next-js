...
export default function CreateAds({ userDetails, adsId }) {
    ...
    async function getEditAds(e) {
        ...
        await axios.post(getPublicUrl + '/api/edit-ad', formdata, config).then(response => {
            // console.log('SKill Data : ', response.data.adsdata);

            if( response.data.adsdata ) {
                // Set Ads Pricing
                if( response.data.adsdata.show_price === 'both' ){
                    setSelectedPriceOption('both')
                    setPricePerDay(response.data.adsdata.price_per_day)
                    setPricePerHour(response.data.adsdata.price_per_hour)
                }else if( response.data.adsdata.show_price === 'day' ) {
                    setSelectedPriceOption('day')
                    setPricePerDay(response.data.adsdata.price_per_day)
                }else if( response.data.adsdata.show_price === 'hour' ) {
                    setSelectedPriceOption('hour')
                    setPricePerHour(response.data.adsdata.price_per_hour)
                }

                setSelectedSkill(response.data.adsdata?.skill_id);
                setAdsHeadline(response.data.adsdata?.title);
                setAdsDescription(response.data.adsdata?.description);
                setMap(prevMap => ({
                    ...prevMap,
                    lat: response.data.adsdata?.latitude,
                }))
                setMap(prevMap => ({
                    ...prevMap,
                    lng: response.data.adsdata?.longitudes,
                }))
                setMap(prevMap => ({
                    ...prevMap,
                    fulladdress: response.data.adsdata?.address,
                }))
                setMap(prevMap => ({
                    ...prevMap,
                    locality: response.data.adsdata?.city,
                }))
                setMap(prevMap => ({
                    ...prevMap,
                    administrative_area_level_1: response.data.adsdata?.state,
                }))
                setMap(prevMap => ({
                    ...prevMap,
                    country: response.data.adsdata?.country,
                }))
                setMap(prevMap => ({
                    ...prevMap,
                    postal_code: response.data.adsdata?.pincode,
                }))
            }
    ...
    async function handleUpdate(e) {
        ...
        console.log('Update Ads from data : ', formdata);
        await axios.post(getPublicUrl+'/api/update-ad', formdata, config).then(response => {
    ...
    return (
        <>
            <button type={`button`} className="edit-button" data-bs-toggle="modal" data-bs-target={`#editAdsModal-${adsId}`} onClick={(e) => getEditAds(e)}>
                <img src={`${getPublicUrl}/images/add-icon.png`} alt="edit-skill" />
            </button>

            {/*Skill Select Modal*/}
            <div className="modal fade" id={`editAdsModal-${adsId}`} tabIndex="-1" aria-labelledby={`editAdsModal-${adsId}`} aria-hidden="true">
                <div className="modal-dialog modal-dialog-scrollable modal-dialog-centered">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h1 className="modal-title fs-5" id="exampleModalLabel">Redigera Annons</h1>
                            ...
                            <Form onSubmit={handleUpdate}>
                                ...
                                <div className={`row`}>
                                    ...
                                    <div className={`user-froms-btns d-flex justify-content-center gap-4`}>
                                        <button type="button" className="no-button outline-primary" data-bs-dismiss="modal" aria-label="Close">Annullera</button>
                                        <button type="submit" className="save-button outline-primary">Uppdatering</button>
                                    </div>
                                </div>
                            </Form>
                        </div>
