...
@section('head-title', 'Edit Badge')

@section('content')
<div class="row">
    <section class="col-lg-6 offset-lg-3">
        <div class="card mb-4">
            <form class="card-body" method="post" action="{{ route('administrator.backend_update_badge', ['id' => $editBadge->id]) }}" enctype="multipart/form-data">
                @csrf
                <div class="row g-3">
                    <div class="col-md-12">
                        <label class="form-label" for="badge_name">Name</label>
                        <input type="text" name="badge_name" id="badge_name" class="form-control @error('badge_name') is-invalid @enderror" placeholder="Abc" value="{{ old('badge_name') ?? $editBadge->badge_name }}"/>
                        @error('badge_name')
                            <span id="badge_name-error" class="error invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-6 ">
                        <img class="img-fluid rounded my-4 justify-content-center " src="{{asset('storage/badges/'.$editBadge->image)}}" height="110" width="110" alt="no_img" id="uploadedBadgeImg"/>
                        ...
                        <div class="button-wrapper">
                            ...
                            <button type="button" class="btn btn-label-secondary badge-image-reset mb-2" {{ empty($editBadge->image) ? 'disabled' : ''}}>
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="mt-1 mb-0">
                            <label class="switch switch-lg">
                                <input type="checkbox" class="switch-input" type="checkbox" name="status" id="status" value="{{ $editBadge->status }}" {{ $editBadge->status == 1 ? 'checked' : '' }}>
    ...
    $(document).ready(function(){
        ...
        if (e) {
            ...
            l.onchange = () => {
                ...
                if( l.value == "") {
                    e.src = '{{asset('storage/badges/no_img.jpeg')}}'
                }
            }
        }
