...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        try {
            ...
            $fetchSkills = Spskill::with('getSkill')->where('user_id',$request['id'] )->first();
            ...
            $skills = null;
            if( !empty( $fetchSkills ) ) {
                if( !empty( $fetchSkills->getSkill ) ) {
                    $skills = $fetchSkills->getSkill;
                }
            }
    ...
    public function search(Request $request)
    {
        ...
        foreach ($records->items() as $record) {
            // $avatarId = Chatify::getUserWithAvatar($record);
